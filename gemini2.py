...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS markets
                 (market_id INTEGER PRIMARY KEY AUTOINCREMENT, question TEXT, creator_id INTEGER, created_at TIMESTAMP, closes_at TIMESTAMP, status TEXT DEFAULT 'pending', result TEXT, pool_yes REAL DEFAULT 0, pool_no REAL DEFAULT 0, fee_collected REAL DEFAULT 0, message_id INTEGER DEFAULT 0)''')
...
def get_market_color(prob_yes: float):
    if prob_yes >= 65: return 0x00FF00
    if prob_yes <= 35: return 0xFF0000
    return 0xFFD700
...
async def get_market_embed(market_id):
    ...
    embed.add_field(
        name=f"#{market_id}: {q}", 
        value=f"{bar}\nYES: {int(prob)}% | NO: {int(100-prob)}%\nЗакрытие: <t:{int(closes.timestamp())}:R>", 
        inline=False
    )
    embed.set_footer(text=f"ID: {market_id} • Жми кнопки ниже")
    return embed
...
def send_proposal_to_tg(question, author_name, price, discord_link):
    ...
    text = (f"📩 *Новая заявка на маркет!*\n\n❓ *Вопрос:* {safe_q}\n👤 *Автор:* {safe_author}\n💰 *Цена:* {price} pts\n\n🔗 [Перейти к кнопкам в Discord]({discord_link})")
...
async def show_portfolio(interaction: discord.Interaction):
    ...
    conn.close()
    embed = discord.Embed(title="💼 Ваше Портфолио", description=f"Баланс: **{int(bal)}** pts", color=discord.Color.blurple())
    if not rows: 
        embed.add_field(name="Пусто", value="У вас нет активных ставок.")
    ...
    for mid, shares, pos, avg, q, py, pn in rows:
        ...
        label = f"#{mid} {q[:15]}..."
        desc = f"{pos.upper()} {int(shares)} шт. | PnL: {int(pnl)}"
...
class BuyModal(discord.ui.Modal):
    def __init__(self, market_id, position):
        super().__init__(title=f"Ставка на {position.upper()}")
        self.market_id = market_id
        self.position = position
        self.amount_input = discord.ui.TextInput(label="Сумма (pts)", placeholder="Например: 100", min_length=1, max_length=6)
        self.add_item(self.amount_input)
    async def on_submit(self, interaction: discord.Interaction):
        try: amount = int(self.amount_input.value)
        except: return await interaction.response.send_message("❌ Введите число!", ephemeral=True)
        if amount <= 0: return await interaction.response.send_message("❌ Сумма должна быть > 0", ephemeral=True)
        user_id = interaction.user.id
        if get_balance(user_id) < amount: return await interaction.response.send_message("❌ Недостаточно средств", ephemeral=True)
        conn = get_db_connection()
        c = conn.cursor()
        ...
        if not m or m[0] != 'active': 
            conn.close()
            return await interaction.response.send_message("❌ Маркет закрыт", ephemeral=True)
        py, pn = m[1], m[2]
        ...
        embed = discord.Embed(title="✅ Ставка принята", color=discord.Color.green())
        embed.add_field(name="Сумма", value=f"{amount} pts на {self.position.upper()}")
        embed.add_field(name="Куплено акций", value=f"{shares:.1f}")
        embed.add_field(name="Новый шанс YES", value=f"{int(new_prob*100)}%")
...
class SellModal(discord.ui.Modal):
    def __init__(self, market_id, position, max_shares):
        super().__init__(title=f"Продать {position.upper()}")
        self.market_id = market_id
        self.position = position
        self.max_shares = max_shares
        self.amount_input = discord.ui.TextInput(label=f"Акции (Макс: {int(max_shares)})", placeholder="0 чтобы продать ВСЁ", default="0")
        self.add_item(self.amount_input)
    async def on_submit(self, interaction: discord.Interaction):
        try: amount_shares = float(self.amount_input.value)
        except: return await interaction.response.send_message("❌ Неверное число", ephemeral=True)
        user_id = interaction.user.id
        conn = get_db_connection()
        c = conn.cursor()
        ...
        if shares_to_sell > self.max_shares:
            conn.close()
            return await interaction.response.send_message("❌ У вас нет столько акций.", ephemeral=True)
        if self.position == "yes":
        ...
        conn.commit()
        conn.close()
        await interaction.response.send_message(f"💸 Продано {shares_to_sell:.1f} {self.position.upper()}. Получено **{int(cash_out)} pts**.", ephemeral=True)
...
class ProposeModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title="Создание нового маркета")
        self.question = discord.ui.TextInput(label="Вопрос", placeholder="Биткоин пробьет $100k?", min_length=5, max_length=100)
        self.hours = discord.ui.TextInput(label="Часов (длительность)", placeholder="48", default="48", min_length=1, max_length=3)
        self.add_item(self.question)
        self.add_item(self.hours)
    async def on_submit(self, interaction: discord.Interaction):
        try: hours_val = int(self.hours.value)
        except: return await interaction.response.send_message("❌ Часы должны быть числом!", ephemeral=True)
        ...
        if not is_vip:
            if hours_val < MIN_DURATION_HOURS or hours_val > MAX_DURATION_HOURS:
                conn.close()
                return await interaction.response.send_message(f"❌ Длительность: {MIN_DURATION_HOURS}-{MAX_DURATION_HOURS} ч.", ephemeral=True)
            if last_prop and now - last_prop < datetime.timedelta(hours=PROPOSAL_COOLDOWN):
                conn.close()
                return await interaction.response.send_message("⏳ Кулдаун!", ephemeral=True)
            if balance < PROPOSAL_COST:
                conn.close()
                return await interaction.response.send_message(f"❌ Нужно {PROPOSAL_COST} pts", ephemeral=True)
        ...
        try:
            ...
            embed.add_field(name="Вопрос", value=question_val)
...
class SellSelect(discord.ui.Select):
    def __init__(self, options):
        super().__init__(placeholder="Выберите позицию для ПРОДАЖИ...", min_values=1, max_values=1, options=options)
...
class MarketControls(discord.ui.View):
    ...
    # --- КНОПКА ПРОДАЖИ ---
    @discord.ui.button(label="💰 Продать", style=discord.ButtonStyle.primary, row=1)
    async def sell_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not positions:
            return await interaction.response.send_message("❌ У вас нет акций в этом маркете.", ephemeral=True)

        if len(positions) == 1:
            pos, shares = positions[0]
            if shares <= 0.01: 
                return await interaction.response.send_message("❌ Акций слишком мало.", ephemeral=True)
            await interaction.response.send_modal(SellModal(self.market_id, pos, shares))
        else:
            options = []
            for pos, shares in positions:
                if shares > 0.01:
                    options.append(discord.SelectOption(label=f"Продать {pos.upper()}", description=f"Доступно: {int(shares)}", value=f"{self.market_id}:{pos}:{shares}"))
            await interaction.response.send_message("Выберите, что продать:", view=PortfolioView(options), ephemeral=True)

    @discord.ui.button(label="📉 График", style=discord.ButtonStyle.secondary, row=1)
    async def chart_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        res = c.fetchone()
        conn.close()
        if not res: return await interaction.followup.send("Маркет не найден")
        buf = generate_chart(self.market_id, res[0])
        if not buf: return await interaction.followup.send("Нет данных")
...
class DashboardView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
    @discord.ui.button(label="📝 Предложить Маркет", style=discord.ButtonStyle.primary, custom_id="dash_propose")
    ...
    @discord.ui.button(label="💼 Портфолио / Продать", style=discord.ButtonStyle.secondary, custom_id="dash_portfolio")
...
class ApprovalView(discord.ui.View):
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not res or res[0] != 'pending':
            conn.close()
            return await interaction.response.send_message("⚠️ Уже обработано.", ephemeral=True)
        closes_at = res[1]
        ...
        await interaction.response.edit_message(content=f"✅ **ОДОБРЕНО** админом {interaction.user.mention}", view=self)
        try:
            user = await interaction.client.fetch_user(self.creator_id)
            await user.send(f"🚀 Ваш маркет **«{self.question}»** одобрен!")
        except: pass
    ...
    async def deny(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        conn.close()
        for child in self.children: child.disabled = True
        await interaction.response.edit_message(content=f"❌ **ОТКЛОНЕНО**", view=self)
...
class PredictionBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        conn.close()
        print(f"🔄 Восстанавливаю кнопки для {len(active_markets)} активных маркетов...")
...
@bot.tree.command(name="setup_dashboard", description="[ADMIN] Создать панель управления")
async def setup_dashboard(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="🔮 Рынок Предсказаний", description="Управляйте своими ставками и создавайте новые события!", color=discord.Color.blurple())
    embed.add_field(name="Как это работает?", value="1. Создайте событие за поинты\n2. Другие делают ставки (YES/NO)\n3. Выигрывайте и попадайте в топ!", inline=False)
...
async def top(interaction: discord.Interaction):
    ...
    conn.close()
    if not rows: 
        embed = discord.Embed(title="📭 Список пуст", description="Напишите **/daily**, чтобы попасть в топ!", color=discord.Color.red())
        return await interaction.response.send_message(embed=embed, ephemeral=True)
    desc = ""
    for i, (uid, bal) in enumerate(rows, 1):
        medal = "🥇" if i==1 else "🥈" if i==2 else "🥉" if i==3 else f"{i}."
        desc += f"**{medal}** <@{uid}> — **{int(bal)} pts**\n"
    embed = discord.Embed(title="🏆 Доска Почета", description=desc, color=discord.Color.purple())
    await interaction.response.send_message(embed=embed)

@bot.tree.command(name="markets", description="Список активных маркетов")
async def markets(interaction: discord.Interaction):
    ...
    rows = c.fetchall()
    conn.close()
    if not rows: return await interaction.response.send_message("Нет активных маркетов.", ephemeral=True)
    await interaction.response.send_message("🔥 **Активные Маркеты**", ephemeral=True)
    for (mid,) in rows:
...
@bot.tree.command(name="portfolio", description="Портфолио")
...
async def daily(interaction: discord.Interaction):
    ...
    if res and res[0] and now - res[0] < datetime.timedelta(hours=24):
        ...
        conn.close()
        return await interaction.response.send_message(f"⏳ Жди <t:{nxt}:R>", ephemeral=True)
    update_balance(uid, 100)
    c.execute('UPDATE users SET last_claim = ? WHERE user_id = ?', (now, uid))
    conn.commit()
    conn.close()
    await interaction.response.send_message("💰 +100 поинтов!", ephemeral=True)

@bot.tree.command(name="balance", description="Баланс")
async def bal(interaction: discord.Interaction):
    await interaction.response.send_message(f"💳 Твой баланс: **{int(get_balance(interaction.user.id))}** pts", ephemeral=True)

@bot.tree.command(name="resolve", description="[ADMIN] Закрыть маркет")
...
async def resolve(interaction: discord.Interaction, market_id: int, winner: app_commands.Choice[str]):
    if not interaction.user.guild_permissions.administrator: return await interaction.response.send_message("Только для админов", ephemeral=True)
    conn = get_db_connection()
    c = conn.cursor()
    ...
    if not st or st[0] != 'active': 
        conn.close()
        return await interaction.response.send_message("Нельзя закрыть этот маркет.", ephemeral=True)
    win = winner.value
    ...
    conn.commit()
    conn.close()
    await interaction.response.send_message(f"✅ Маркет #{market_id} закрыт. Победил: {win.upper()}. Выплачено: {int(total)} pts.")
